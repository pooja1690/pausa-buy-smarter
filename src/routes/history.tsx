      ...
      {records.length === 0 ? (
        ...
        <ul className="space-y-3 pb-8">
          {records.map((r) => (
            <li
              key={r.id}
              className="rounded-2xl bg-card border border-border p-4 fade-up"
            >
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="rounded-full bg-muted px-3 py-1.5 text-xs font-medium hover:bg-border"
                        >
                          Yes
                        </button>
                        <button
                          onClick={() => setBought(r.id, false)}
                          className="rounded-full bg-primary-soft text-primary px-3 py-1.5 text-xs font-medium hover:opacity-90"
                        >
                          No
                        </button>
                      </div>
...
function EmptyState() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-20">
      ...
      <p className="text-foreground">No decisions yet</p>
      <p className="text-sm text-muted-foreground mt-1">
        Your past pauses will appear here.
      </p>
      <Link
        to="/"
        className="mt-6 rounded-2xl bg-primary text-primary-foreground px-5 py-3 text-sm font-medium"
      >
        Take a pausa
      </Link>
    </div>
